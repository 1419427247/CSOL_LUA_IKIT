...
elseif UI then
    ...
    local frame = IKit.New("Frame"):add(IKit.New("Plane","0_1",80,0,20,100));
    
    local selectbox2 = IKit.New("SelectBox","1_2",2,2,96,10,{"页数/1"});
    ...
    frame:findByTag("0_1"):add(selectbox2,button4,button5,button6,button7,button8);
    ...
    Event:addEventListener("OnKeyDown",function(inputs)
        if inputs[UI.KEY.K] == true then
            if frame.isvisible then
                ...
                frame:findByTag("0_1"):animate({"style.left",80},50);
            end
        end
    end);
